...
def checkerror():
	for n in AUP_ok:
		if AUP_ok[n] == False:
			tkMessageBox.showerror("Arquivos .aup deifeituosos", n + " apresenta erros de digitação - Confira novamente as amostras " )
...
d=Celula (root,3,sexo,cor4)
e=Celula (root,4,historia,cor5)
f=Celula (root,5,gramatica,cor6)
...
def filtraSujeito():
	for celula_cheia in [a.CELULAtual(),b.CELULAtual()]:
...
GERAR.add_command(label="Média entre os selecionados abaixo", command=filtraSujeito)
